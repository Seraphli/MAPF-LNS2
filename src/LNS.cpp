  ...
  while (p != shuffled_agents.end() &&
    ...
    if (screen >= 3)
      cout << "Remaining agents = " << remaining_agents
           << ", remaining time = " << T - ((fsec)(Time::now() - time)).count()
           << " seconds. " << endl
...
void LNS::randomWalk(int agent_id, int start_location, int start_timestep,
  ...
  for (int t = start_timestep; t < upperbound; t++) {
    ...
    while (!next_locs.empty()) {
      ...
      {
        path_table.getConflictingAgents(agent_id, conflicting_agents, loc, *it,
                                        t + 1);
        loc = *it;
        break;
      }
...
void LNS::validateSolution() const {
  ...
  for (const auto &a1_ : agents) {
    ...
    for (const auto &a2_ : agents) {
      ...
      for (; t < (int)a1.path.size(); t++) {
        ...
        {
          cerr << "Find a vertex conflict between agents " << a1.id << " and "
               << a2.id << " at location " << a1.path[t].location
               << " at timestep " << t << endl;
          exit(-1);
        ...
        {
          cerr << "Find an edge conflict between agents " << a1.id << " and "
               << a2.id << " at edge (" << a1.path[t - 1].location << ","
      ...
      for (; t < (int)a2.path.size(); t++) {
        if (a2.path[t].location == target) // target conflict
        {
          cerr << "Find a target conflict where agent " << a2.id
               << " (of length " << a2.path.size() - 1 << ") traverses agent "
               << a1.id << " (of length " << a1.path.size() - 1
               << ")'s target location " << target << " at timestep " << t
               << endl;
          exit(-1);
        }
      }
  ...
  if (sum_of_costs != sum) {
    cerr << "The computed sum of costs " << sum_of_costs
         << " is different from the sum of the paths in the solution " << sum
         << endl;
    exit(-1);
  }
}
...
void LNS::writeResultToFile(const string &file_name) const {
  if (init_lns != nullptr) {
    init_lns->writeResultToFile(file_name + "-initLNS.csv", sum_of_distances,
                                preprocessing_time);
  }
  string name = file_name;
  if (use_init_lns or num_of_iterations > 0)
  ...
  if (!exist) {
    ...
    addHeads << "runtime,solution cost,initial solution cost,lower bound,sum "
                "of distance,"
             << "iterations,"
             << "group size,"
  ...
  if (!iteration_stats.empty()) {
    ...
    auc +=
        (prev->sum_of_costs - sum_of_distances) * (time_limit - prev->runtime);
  }
